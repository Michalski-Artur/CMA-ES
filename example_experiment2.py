...
from cma import CMA
from cma_fmin import fmin as cma_fmin
from cma_fmin import fmin_random as cma_fmin_random
from cma_fmin import fmin_square_root as cma_fmin_square_root
...
for batch_counter, problem in enumerate(suite):  # this loop may take hours or days...
    ...
    while evalsleft() > 0 and not problem.final_target_hit:
        ...
        elif fmin.__name__ == 'fmin':
            xopt, es = cma_fmin(problem=problem, mean=propose_x0(), sigma=2)
        elif fmin.__name__ == 'fmin_random':
            xopt, es = cma_fmin_random(problem=problem, mean=propose_x0(), sigma=2)
            stoppings[problem.index].append(es.should_stop())
        elif fmin.__name__ == 'fmin_square_root':
            xopt, es = cma_fmin_square_root(problem=problem, mean=propose_x0(), sigma=2)
